...
export function ResultsScreen({
  ...
  useEffect(() => {
    ...
    const cell = container?.querySelector('[data-active="true"]');
    if (!container || !cell) return;
    const item = cell.getBoundingClientRect();
    const viewport = container.getBoundingClientRect();
    // Move only the filmstrip, never the page or an ancestor scroll container.
    container.scrollBy({
      left: item.left - viewport.left - (viewport.width - item.width) / 2,
  ...
  useEffect(() => {
    ...
    if (!node) return;
    const start = () => {
      node.currentTime = trimStart + active.start;
      void node.play().catch(() => {});
    };
    const limit = () => {
      if (
        node.currentTime >= trimStart + duration - 0.03 ||
    ...
    return () => {
      ...
      node.removeEventListener("timeupdate", limit);
    };
  ...
  return (
    <Screen className="results-screen">
      <div className="flex shrink-0 items-center justify-between">
        ...
        >
          <ArrowLeft size={17} />
          {newVideoLabel ?? copy.results.newVideo}
        </button>
      ...
      <div className="photo-stage">
        ...
        />
        <span className="pointer-events-none absolute left-4 top-4 rounded-full bg-black/40 px-3 py-1.5 font-mono text-[10px] text-white">
          {formatSeconds(active.start)}
        </span>
        <button
          className="absolute right-3 top-3 grid h-11 w-11 place-items-center rounded-full bg-black/40 text-white"
          aria-label={copy.studio.selectPhoto + " — " + active.title}
          aria-pressed={checked.has(selected)}
          onClick={() => onToggleCheck(selected)}
        >
          <span
            className={`grid h-6 w-6 place-items-center rounded-full border ${checked.has(selected) ? "border-[#d4ed8a] bg-[#d4ed8a] text-[#25352d]" : "border-white/80"}`}
          >
            {checked.has(selected) && <Check size={15} />}
          </span>
        </button>
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-2 bg-gradient-to-t from-black/75 to-transparent p-4 pt-12 text-white">
        ...
        <div className="filmstrip" ref={strip}>
          {highlights.map((item, index) => (
            <button
              ...
              aria-label={
                copy.results.moment + " " + (index + 1) + ": " + item.title
              }
            ...
            >
              ...
              />
              <span className="absolute bottom-1 left-1 rounded-md bg-black/45 px-1.5 text-[10px] text-white">
                {String(index + 1).padStart(2, "0")}
              </span>
              {checked.has(index) && (
                <span className="absolute right-1 top-1 rounded-full bg-[#d4ed8a] p-0.5 text-[#25352d]">
                  <Check size={12} />
                </span>
              )}
            </button>
      ...
      <div className="grid shrink-0 grid-cols-[50px_1fr] gap-2">
        ...
        <button
          ...
          onClick={() =>
            checked.size ? onDownloadChecked() : onDownloadOne(active)
          }
        >
          <Download size={18} />
          {checked.size
            ? copy.results.download + " (" + checked.size + ")"
            : copy.results.downloadOne}
        </button>
      </div>
      {preview && (
        <Sheet
        ...
        >
          {preview === "video" ? (
            <video
              ...
              controls
              playsInline
              className="max-h-[65dvh] w-full rounded-2xl bg-black"
            />
          ) : (
            // eslint-disable-next-line @next/next/no-img-element
            <img

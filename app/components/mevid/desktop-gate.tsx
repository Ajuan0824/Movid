...
export function DesktopGate({ copy }: DesktopGateProps) {
  return (
    <main className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden bg-[#f8f7fb] px-6 text-center text-[#232331]">
      <DotGrid />
      ...
      >
        ...
        <h1 className="font-display text-2xl font-bold tracking-[-0.04em] text-[#232331]">{copy.desktopGate.title}</h1>
        <p className="text-sm leading-6 text-[#6d6b79]">{copy.desktopGate.description}</p>
      </motion.section>
    </main>
  );
}

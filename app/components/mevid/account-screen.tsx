...
export function AccountScreen({ copy, onGoPro }: AccountScreenProps) {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={screenTransition} className="mx-auto flex w-full max-w-lg flex-1 flex-col py-3">
      ...
      {canChangePassword ? (
        <div className="glass-panel mt-3 rounded-[26px] p-4 shadow-panel">
          <p className="font-display text-xl font-bold tracking-[-0.03em]">{t.passwordTitle}</p>
          <p className="mt-1 text-sm text-[#6d6b79] dark:text-[#a79fb5]">{t.passwordDescription}</p>
          <div className="mt-4 flex flex-col gap-3">
        ...
        <p className="mt-3 rounded-2xl bg-white/60 dark:bg-white/5 px-4 py-3.5 text-sm leading-5 text-[#6d6b79] dark:text-[#a79fb5]">{t.noPasswordProvider}</p>
      )}

  ...
  return (
    <motion.section initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={screenTransition} className="mx-auto flex w-full max-w-lg flex-1 flex-col items-center justify-center py-16 text-center">
      ...
      <motion.div variants={heroTextVariants} initial="hidden" animate="visible">
        <motion.div variants={heroTextItemVariants} className="mb-4 inline-flex items-center gap-2 rounded-full bg-[#f0ecff] px-3 py-1.5 text-xs font-bold text-[#7657dd]"><LoaderCircle size={13} className="animate-spin" />{copy.analysis.eyebrow}</motion.div>
        ...
        <motion.p variants={heroTextItemVariants} className="mt-4 min-h-6 text-sm font-medium text-[#777481]">{copy.analysis.steps[step]}</motion.p>
      </motion.div>

...
export function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mb-8">
      ...
      <div className="flex flex-col gap-2.5 text-[15px] leading-6 text-[#53604e] dark:text-[#d1dbc9]">{children}</div>
    </section>
  );
}
...
export function Mail() {
  return (
    <a href={`mailto:${CONTACT_EMAIL}`} className="font-semibold text-[#466447] underline underline-offset-2 dark:text-[#d4ed8a]">
      {CONTACT_EMAIL}
    </a>
  );
}
